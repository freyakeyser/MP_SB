...
receiver <- read.csv("/Users/freyakeyser/Documents/Acadia/ACER/Masters/Overall spreadsheets:figures/Receiver metadata.csv")
...
#tagging_2011 <- subset(tagging, year=="2011")
tagging_2012 <- subset(tagging, year=="2012")
```
...
MP_2012_join_summer <- join(MP_2012_summer, coords, type="full")
MP_2012_join_winter <- join(MP_2012_winter, coords, type="full")
...
reccounts_dd_2012_summer <- reccounts_dd_2012_summer[1:127,]
...
map_sp <- readShapeSpatial("/Users/freyakeyser/Desktop/Shapefiles 2/New_Polygon.shp")
map <- fortify(map_sp)
SP <- SpatialPoints(cbind(map$long, map$lat),
                    proj4string=CRS("+proj=utm +zone=20 +datum=WGS84 +units=m +no_defs "))
SP2 <- spTransform(SP, CRS("+proj=longlat"))
...
MP_2012_depths$diffs<-c(NA, (MP_2012_depths$Date.and.Time..UTC.[2:8632] - MP_2012_depths$Date.and.Time..UTC.[1:8631]))
  
MP_2012_depths$inds<-c(NA, MP_2012_depths$Fish.code[2:8632]==MP_2012_depths$Fish.code[1:8631])

MP_2012_depths$trans<-c(NA, MP_2012_depths$Detection.depth[2:8632]-MP_2012_depths$Detection.depth[1:8631]==0)
...
#write.csv(detects_final_MP, "/Users/freyakeyser/Documents/Acadia/ACER/Masters/Overall spreadsheets:figures/Passage/Filtered MP detections.csv")
...
densityforce <- ggplot() + 
  geom_density(data=detects_final_force, aes(Detection.depth)) +
  coord_flip() +
  scale_x_reverse(limits=c(125, 0), breaks=c(0, 25, 50, 75, 100, 125))+
  figure_theme +
  xlab("Detection depth (m)") +
  ylab("Density") +
  ggtitle("FORCE 2011 and 2012") 
...
print(density2011, vp = viewport(layout.pos.row = 1, layout.pos.col = 3)) 
print(density2012_summer, vp = viewport(layout.pos.row = 2, layout.pos.col = 3))
print(density2012_winter, vp = viewport(layout.pos.row = 2, layout.pos.col = 4))
print(densityforce, vp = viewport(layout.pos.row = 1, layout.pos.col = 4))
...
#       panel.grid.minor = element_blank(), panel.margin = unit(1, "lines")) +
  coord_flip() +
  scale_x_reverse() +
  geom_text(data=detects_final_MP_summary, aes(x=11, y=0.1, label=n), 
                    colour="black", inherit.aes=FALSE, parse=FALSE, hjust=1)+
  geom_text(data=detects_final_MP_summary_2, aes(x=1, y=0.1, label=n), 
                    colour="black", inherit.aes=FALSE, parse=FALSE, hjust=1) +
  figure_theme
